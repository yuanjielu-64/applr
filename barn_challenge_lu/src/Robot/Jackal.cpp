...
void Robot_config::globalPathCallback(const nav_msgs::Path::ConstPtr &msg) {
    ...
    if ((int) msg->poses.size() == 0) {
        if (local_goals_history.size() >= 2){
            std::vector<double> lg = transform_lg(local_goals_history[0][0], local_goals_history[0][1],
                robot_state.x_, robot_state.y_, robot_state.theta_);

            setLocalGoal(lg, local_goals_history[0][0], local_goals_history[0][1]);

            local_paths_history.erase(local_paths_history.begin());
            local_goals_history.erase(local_goals_history.begin());

            view_Goal(global_goal_odom, local_goal_odom);

            return;
        }

        if (!local_paths_history.empty()) {
            ...
            double threads = 1;
            for (size_t i = 0; i < local_paths_history[0].size(); ++i) {
                lg = transform_lg(local_paths_history[0][i][0], local_paths_history[0][i][1], robot_state.x_, robot_state.y_, robot_state.theta_);
            ...
            std::vector<std::vector<double>> paths = {{robot_state.x_, robot_state.y_}};
            for (size_t i = close_id; i < X.size(); ++i) {
                std::vector<double> vector = {local_paths_history[0][i][0], local_paths_history[0][i][1]};
                paths.push_back(vector);
            }

            local_paths_odom = paths;
            ...
            for (size_t i = close_id; i < X.size(); ++i) {
                ...
                if (length >= std::max(threads - 0.08 * re, 0.2) && flag == false) {
                    lg = {X[i], Y[i]};
                    setLocalGoal(lg, local_paths_history[0][i + close_id][0], local_paths_history[0][i + close_id][1]);
                    flag = true;
                    // ROS_INFO("aaaaaaaaaa: %f, %f, %f, %f,  %f, %f, %f", local_paths_history[0][i][0], local_paths_history[0][i][1], robot_state.x_, robot_state.y_, length, X[i], Y[i]);
                    break;
                }
            }
    ...
    if (!local_paths.empty() && !local_goal_odom.empty()) {

        local_paths_history.push_back(local_paths_odom);
        local_goals_history.push_back(local_goal_odom);

        if (local_paths_history.size() > 30) {
            local_paths_history.erase(local_paths_history.begin());
        }

        if (local_goals_history.size() > 30) {
            local_goals_history.erase(local_goals_history.begin());
        }
...
bool Robot_config::isPaused() const {
    ...
    switch(currentState) {
        ...
        case IDIE: state_msg.data = "IDIE"; break;
...
void Robot_config::paramsCallback(const std_msgs::Float64MultiArray::ConstPtr& msg) {
    ...
    if (getAlgorithm() == DWA) {
        ...
        v = msg->data[0];
        w = msg->data[1];
        vx_sample = msg->data[2];
        vTheta_samples = msg->data[3];
        path_distance_bias = msg->data[4];
        goal_distance_bias = msg->data[5];
...
std::vector<double> Robot_config::getSize() const {
    ...
    if (currentMap == ONLY_LASER_RECEIVED && getRobotState() == RECOVERY)
        info = {0.508, 0.430, 0.0, v, -w, w};
    else if (currentMap == ONLY_LASER_RECEIVED && getRobotState() == BACKWARD)
        info = {0.02, 0.02, -2.0, 0.0, -2, 2};
    else if (currentMap == ONLY_LASER_RECEIVED && getRobotState() == FORWARD)
