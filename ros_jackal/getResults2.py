...
def get_hardest_environments(sorted_results, top_n=50):
    ...
    # selected_hard = random.sample(hard_pool, 50)
    selected_hard = hard_pool[:25]
...
def analyze_specific_environments(sorted_results, env_ids):
    """分析指定环境的表现"""
    print(f"\n🎯 分析指定的 {len(env_ids)} 个环境")
    print("=" * 80)
    ...
    for result in sorted_results:
        if result['world_id'] in env_ids:
            target_results.append(result)
            found_env_ids.append(result['world_id'])
    ...
    # 计算统计信息
    avg_times = [r['avg_time'] for r in target_results]
    success_rates = [r['success_rate'] for r in target_results]
    nav_metrics = [r['avg_nav_metric'] for r in target_results]

    overall_avg_time = np.mean(avg_times)
    overall_success_rate = np.mean(success_rates)
    overall_nav_metric = np.mean(nav_metrics)
    ...
    for result in target_results:
        ...
        for status, stats in result['status_stats'].items():
            if status == 'success':
                ...
                total_collision += stats['count']
            elif status == 'timeout':
                total_timeout += stats['count']

    print(f"找到的环境数量: {len(target_results)} / {len(env_ids)}")
    print(f"缺失的环境: {set(env_ids) - set(found_env_ids)}")
    print(f"总测试次数: {total_episodes}")
    print("-" * 40)
    print(f"成功次数: {total_success}")
    print(f"碰撞次数: {total_collision}")
    print(f"超时次数: {total_timeout}")
    print(f"成功率: {(total_success / total_episodes * 100):.2f}%")
    print(f"碰撞率: {(total_collision / total_episodes * 100):.2f}%")
    print(f"超时率: {(total_timeout / total_episodes * 100):.2f}%")
    print(f"平均时间: {overall_avg_time:.4f}秒")
    print(f"导航指标平均值: {overall_nav_metric:.4f}")

    print(f"\n各环境详细情况:")
    print("-" * 80)
    print(f"{'环境ID':<8} {'成功率':<10} {'平均时间':<12} {'导航指标':<12} {'主要失败原因':<15}")
    print("-" * 80)
